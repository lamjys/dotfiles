...
FIELDS=(symbol marketState regularMarketPrice regularMarketChange regularMarketChangePercent \
    preMarketPrice preMarketChange preMarketChangePercent postMarketPrice postMarketChange postMarketChangePercent currency shortName)
...
for symbol in $(IFS=' '; echo "${SYMBOLS[*]}" | tr '[:lower:]' '[:upper:]'); do
  ...
  if [ "$price" != "null" ]; then
    # printf "%-10s%-25s%-4s$COLOR_BOLD%'8.2f$COLOR_RESET" $symbol "$shortName" $currency $price
    (LC_ALL='en_US.UTF-8'; printf "%-10s%-25s%-4s$COLOR_BOLD%'10.2f$COLOR_RESET" $symbol "$shortName" $currency $price)
