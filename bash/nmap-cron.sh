#!/bin/bash
# one liner
nmap -F -T4 --min-rate 10 192.168.0.141 -oX - | xq -r '.nmaprun.runstats.hosts."@up"' | sed "s/^/,/g" | ts "%Y-%m-%d %H:%M:%S" | tee >(mosquitto_pub -h 192.168.0.210 -u lamjys -P $(base64 -d ~/.config/mqtt-base) -t nmap -l) >(tee -a ~/Documents/nmap.log > /dev/null)
